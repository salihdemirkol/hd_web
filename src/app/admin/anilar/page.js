            ...
            {items.length === 0 ? <p style={{ color: '#888' }}>Henüz anı eklenmemiş.</p> : items.map((item) => (
              <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem', backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '8px', border: '1px solid #333', opacity: item.hidden ? 0.5 : 1 }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <a href="/anilar" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', padding: '0.5rem 1rem', backgroundColor: '#218c74', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center' }}>Gör</a>
