...
export default function AdminMakaleler() {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editId, setEditId] = useState(null);
  const [formData, setFormData] = useState({ title: '', content: '', date: '', source: '' });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setFormData({ title: '', content: '', date: '', source: '' });
      alert(isEdit ? 'Makale güncellendi!' : 'Makale başarıyla eklendi!');
    } catch (error) {
      alert(error.message);
    } finally {
      setSaving(false);
  ...
  const handleEdit = (item) => {
    setEditId(item.id);
    setFormData({ title: item.title, content: item.content, date: item.date, source: item.source || '' });
  ...
  return (
    <div>
      <h1 style={{ fontSize: '2rem', marginBottom: '2rem', color: '#fff' }}>Makale Yönetimi</h1>
      ...
      <div className="glass-panel" style={{ backgroundColor: 'rgba(25,25,25,0.6)', padding: '2rem', borderRadius: '16px', border: '1px solid #333', marginBottom: '2rem' }}>
        <h2 style={{ marginBottom: '1.5rem', color: 'var(--color-secondary)', fontSize: '1.25rem' }}>{editId ? 'Makaleyi Düzenle' : 'Yeni Makale Ekle'}</h2>
        <form onSubmit={handleSave} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#aaa' }}>Makale Başlığı</label>
          ...
          <div>
            ...
            <input type="text" placeholder="Örn: 15 Ağustos 2021" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} required style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', backgroundColor: 'rgba(0,0,0,0.5)', border: '1px solid #333', color: '#fff' }} />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#aaa' }}>Kaynak (Opsiyonel)</label>
            <input type="text" placeholder="Örn: Milli Gazete" value={formData.source} onChange={e => setFormData({...formData, source: e.target.value})} style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', backgroundColor: 'rgba(0,0,0,0.5)', border: '1px solid #333', color: '#fff' }} />
          </div>
          <div style={{ gridColumn: '1 / -1', marginTop: '1rem' }}>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: '#aaa' }}>Makale İçeriği</label>
          ...
          <div style={{ gridColumn: '1 / -1', marginTop: '1rem', display: 'flex', gap: '1rem' }}>
            <button type="submit" disabled={saving} style={{ padding: '1rem 2rem', borderRadius: '8px', backgroundColor: 'var(--color-secondary)', color: '#000', fontWeight: 'bold', border: 'none', cursor: saving ? 'not-allowed' : 'pointer' }}>
              {saving ? 'Kaydediliyor...' : (editId ? 'Değişiklikleri Kaydet' : 'Makaleyi Yayınla')}
            </button>
            {editId && (
              <button type="button" onClick={() => { setEditId(null); setFormData({ title: '', content: '', date: '', source: '' }); }} style={{ padding: '1rem 2rem', borderRadius: '8px', backgroundColor: 'transparent', color: '#fff', border: '1px solid #555', cursor: 'pointer' }}>
                İptal Et
              </button>
            )}
          </div>
      ...
      <div className="glass-panel" style={{ backgroundColor: 'rgba(25,25,25,0.6)', padding: '2rem', borderRadius: '16px', border: '1px solid #333' }}>
        <h2 style={{ marginBottom: '1.5rem', color: '#fff', fontSize: '1.25rem' }}>Mevcut Makaleler</h2>
        {loading ? <p>Yükleniyor...</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {items.length === 0 ? <p style={{ color: '#888' }}>Henüz makale eklenmemiş.</p> : items.map((item) => (
              <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem', backgroundColor: item.hidden ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.3)', borderRadius: '8px', border: item.hidden ? '1px dashed #555' : '1px solid #333', transition: 'all 0.3s' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <a href="/makaleler" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', padding: '0.5rem 1rem', backgroundColor: '#218c74', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', opacity: item.hidden ? 0.3 : 1, pointerEvents: item.hidden ? 'none' : 'auto' }}>Gör</a>
